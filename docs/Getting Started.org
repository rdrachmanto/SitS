...
Type the following command on your PC to know your IP addresses: 
...
To connect to the Pi, enter this command, and type the pi's password when prompted. On some OS, there are no characters shown to the terminal when typing the
